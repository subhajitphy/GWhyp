...
def get_hyp_waveform(M,q,et,b,t,inc,distance,phi0='None'):
    if phi0=='None':
        phi0=0
...
def cal_sp_sx_A(M,q,et,b,t,D_GW,inc):
    η=q/(1+q)**2
    Time=M*tsun
    dis=M*dsun
    scale=D_GW/dis
...
def cal_sp_sx(M,q,et,b,tarr,D_GW,inc):
    
    η=1/(1+q)**2
    h=get_hyp_waveform(M, q, et, b,tarr, inc, D_GW)
...
def hyp_pta_res(toas,
    ...
    log10_n     is the log10 n of the GW source
    ...
    s_arr = np.dot([Fp,Fx], np.dot(Rpsi, s_arr))
